(() => {
  ...
  if (glossaryRoot && GLOSSARY.length) {
    ...
    const glossaryParams = new URLSearchParams(window.location.search);
    const requestedGlossaryTerm = glossaryParams.get("dict") || glossaryParams.get("dictionnaire");
    if (requestedGlossaryTerm) {
      const requestedEntry = GLOSSARY.find(item => normalize(item.term) === normalize(requestedGlossaryTerm))
        || GLOSSARY.find(item => (item.aliases || []).some(alias => normalize(alias) === normalize(requestedGlossaryTerm)));
      if (requestedEntry) {
        activeGroup = requestedEntry.group || "all";
        updateGroupButtons();
        requestAnimationFrame(() => renderEntry(requestedEntry,{close:true,updateSearch:true}));
      }
    }
  ...
  const attachTimelineLinks = (title, links) => {
    const heading = [...document.querySelectorAll(".bible-chrono-entry--bible > strong")]
      .find((node) => (node.textContent || "").trim() === title);
    const entry = heading?.closest(".bible-chrono-entry--bible");
    if (!entry || entry.querySelector(".bible-timeline-links")) return;
    const nav = document.createElement("div");
    nav.className = "bible-timeline-links";
    nav.innerHTML = links.map(([href, label]) => `<a href="${href}">${label}</a>`).join("");
    entry.append(nav);
  };

  attachTimelineLinks("Joseph et les Hébreux en Égypte", [
    ["/textes/genese-joseph-vendu-freres/", "Genèse 37 · Joseph vendu par ses frères"],
    ["/textes/genese-ascension-joseph/", "Genèse 41 · Ascension de Joseph"],
    ["/textes/genese-pardon-joseph/", "Genèse 45 · Le pardon de Joseph"],
    ["/textes/genese-heritier-promesse/", "Genèse 49 · L’héritier de la Promesse"]
  ]);
  attachTimelineLinks("Moïse et l’Exode", [
    ["/textes/exode-esclavage-hebreux/", "Exode 1 · L’esclavage des Hébreux"],
    ["/textes/exode-naissance-moise/", "Exode 2 · La naissance de Moïse"],
    ["/textes/exode-appel-moise/", "Exode 3 · L’appel de Moïse"],
    ["/textes/exode-paque-israel/", "Exode 12 · La Pâque d’Israël"],
    ["/textes/exode-delivrance-israel/", "Exode 12–13 · La sortie d’Égypte"],
    ["/textes/exode-passage-mer-rouge/", "Exode 14 · Le passage de la Mer Rouge"],
    ["/textes/exode-cantique-moise/", "Exode 15 · Le cantique de Moïse"],
    ["/textes/exode-alliance-sinai/", "Exode 19 & 24 · L’Alliance du Sinaï"],
    ["/textes/exode-dix-commandements/", "Exode 20 · Les Dix Commandements"]
  ]);
